...
for n in DF["phone"]:
    reg1 = re.findall(r'\d{2}-\d{3}-\d{3}', n)
    reg2 = re.findall(r'[(]\d{3}[)]-\d{3}-\d{4}', n)
    reg3 = re.fullmatch(r'\d*-\d+|\d+-\d*', n)
    reg4 = re.findall(r'\d+ +', n)
    if reg1 != []:
        regex_structure1.append(n)
    elif reg2 != []:
        regex_structure2.append(n)
    elif reg3 != None:
        regex_structure3.append(n)
    elif reg4 != []:
        regex_structure4.append(n)
    else:
        other_numbers.append(n)

print(regex_structure1)
print(regex_structure2)
print(regex_structure3)
print(regex_structure4)
print(other_numbers)
